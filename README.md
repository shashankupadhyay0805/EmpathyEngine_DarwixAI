...
{
  "emotion": "happy",
  "audio_file": "outputs/generated_audio/abc123def456.mp3"
}
```

**Play audio**

- Use the path returned in `audio_file`; the server can serve the file at:
  - `GET /audio/<basename>`  
  e.g. `GET /audio/abc123def456.mp3`
...
User input (text)
       ...
       ▼
┌──────────────────┐
│ emotion_detector │  HuggingFace / VADER / TextBlob
└────────┬─────────┘
         │ emotion + confidence
         ▼
┌──────────────────┐
│   voice_mapper    │  emotion → pitch, rate, volume
└────────┬─────────┘
         │ SpeechParams
         ▼
┌──────────────────┐
│   tts_engine     │  gTTS / ElevenLabs → pydub (apply params)
└────────┬─────────┘
         │
         ▼
